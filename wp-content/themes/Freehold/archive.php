		...
		<div class="width-container">
			...
			<?php if(of_get_option('fw_blog', 'no') == 'no'): ?><div id="container-sidebar"><?php endif; ?>
				<div class="content-boxed">
					<h2 class="title-bg">
						<?php if(is_day()): ?>
							<?php printf(__('Daily Archives: %s', 'progressionstudios'), get_the_date()); ?>
						<?php elseif(is_month()) : ?>
							<?php printf(__('Monthly Archives: %s', 'progressionstudios'), get_the_date('F Y')); ?>
						<?php elseif(is_year()) : ?>
							<?php printf(__('Yearly Archives: %s', 'progressionstudios'), get_the_date('Y')); ?>
						<?php elseif(is_category()): ?>
							<?php printf(__('%s Category', 'progressionstudios'), single_cat_title('', false)); ?>
						<?php elseif(is_tag()): ?>
							<?php printf(__('Tag: %s', 'progressionstudios'), single_cat_title('', false)); ?>
						<?php else: ?>
							<?php _e('Blog Archives', 'progressionstudios'); ?>
						<?php endif; ?>
					</h2>
